...
class Piece(object):
	...
	def get_template(self):
		...
		yield from [(self.x + x, self.y + y) for y in size for x in size if template[y][x] == '#']
	...
	def render(self, console):
		for (x, y) in self.get_template():
			if y >= 0:
				pygame.draw.rect(console, self.color, (scale(8 + shift(x)), scale(8 + shift(y)), scale(6), scale(6)))


class Board(object):
    """ board class """
	__slots__ = ('x', 'y', 'w', 'h', 'grid', 'pieces', 'hold', 'level', 'score', 'fall_speed',
	...
	def __init__(self, x, y, w, h):
		...
		self.grid = [[None for _ in range(w)] for _ in range(h)]  # make an empty board
		self.pieces = [None, Piece(), None, None]  # [current piece, next piece, held piece, ghost piece]
		self.get_next_piece()
		self.get_ghost()
	...
	def get_next_piece(self):
		...
		self.pieces[0].reset()  # set the starting position of the current piece
	...
	def is_valid_position(self, piece):
		...
		for (x, y) in piece.get_template():
			if x < 0 or x > 9 or y > 19 or (0 <= x <= 9 and 0 <= y <= 19 and self.grid[y][x]):
				return False
		return True
	...
	def update(self, inputs):
		...
		if time.time() - self.last_update > 0.08:
			self.last_update = time.time()
			if (self.pieces[0].x, self.pieces[0].y) == (self.pieces[3].x, self.pieces[3].y):
				self.set_piece()
				inputs.reset()
				self.hold = True
		...
		if lines_cleared:
			self.score += MULTIPLIER[lines_cleared] * (self.level + 1)  # update the score

		if self.level < 30:
			self.level = self.score // 1000  # update the level
			self.fall_speed = 0.5 - self.level * 0.015  # update the falling speed of the pieces

		self.get_ghost()

	def render(self, console):
		# render the border
		pygame.draw.rect(console, WHITE, (scale(self.x - 2), scale(self.y - 2), scale(8 * self.w + 4), scale(8 * self.h + 4)), scale(1))

		# render the board
		for y in range(self.h):
			for x in range(self.w):
				if self.grid[y][x]:
					pygame.draw.rect(console, self.grid[y][x], (scale(self.x + shift(x)), scale(self.y + shift(y)), scale(6), scale(6)))
...
class EventHandler(object):
	...
	__slots__ = ('move_left', 'move_right', 'move_down', 'rotate_left', 'rotate_right', 'drop', 'hold')
...
class Engine(object):
    """ game engine class """
	__slots__ = ('console', 'event_handler', 'fps_clock', 'gui', 'state', 'board')
	
	def __init__(self):
		pygame.init()
		pygame.display.set_caption('Tetris')
		self.console = pygame.display.set_mode((scale(WIN_WIDTH), scale(WIN_HEIGHT)))
